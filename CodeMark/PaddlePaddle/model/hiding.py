...
from utils import get_var_from_var_id, get_var_map

from .gat import MyGatModel
from .var_decoder import VarDecoder, VarDecoderGru, VarSelector
from .wm_encoder import WMLinearEncoder


class Hiding(paddle.nn.Layer):
    def __init__(self, config, node_encoder, func_gru=None):
        super(Hiding, self).__init__()
        self.config = config
        self.wm_encoder = WMLinearEncoder(
            config.watermark_len, config.watermark_emb_dims
        )
    ...
    def forward(
        ...
        g_emb = self.encode_graph(
            graph_batch, var_node_index_in_node_batch, watermarks_class
        )
        max_subtoken_len = max(pre_var_tok_lens)
        batch_size = tuple(watermarks_class.shape)[0]
        var_logits = paddle.zeros(
            shape=[max_subtoken_len, batch_size, self.config.vocab_size]
    ...
    def get_func_emb(self, func_token_ids, attention_masks):
        output = self.codebert(
            input_ids=func_token_ids,
    ...
    def concat_wm_func_g_emb(
        self, batch, func_embs, wm_embs, g_embs, only_func_g_emb=False
    ):
        concat_embs = []
        for batch_index, (g_emb_begin, g_emb_end) in enumerate(
            ...
            for g_emb in g_embs[g_emb_begin:g_emb_end, :]:
                if only_func_g_emb:
                    concat_emb = paddle.concat(x=(func_emb, g_emb), axis=0)
                else:
                    concat_emb = paddle.concat(x=(func_emb, wm_emb, g_emb), axis=0)
                concat_embs.append(concat_emb)
        concat_embs = paddle.stack(x=concat_embs, axis=0)
        return concat_embs
    ...
    def get_rename_var_tok_embs(self, batch, var_selctions, pre_var_logits):
        var_selctions = paddle.nn.functional.gumbel_softmax(
            x=paddle.nn.functional.log_softmax(x=var_selctions, axis=-1),
            temperature=0.1,
            hard=True,
        )
        pre_var_tok_embs = self.node_encoder.get_var_emb(pre_var_logits)
        ori_var_ids = batch["var_tok_ids"][:, : tuple(pre_var_logits.shape)[1]]
        ori_var_tok_embs = self.node_encoder.embed_node(ori_var_ids)
        rename_var_tok_embs = []
        for var_index in range(len(batch["vars"])):
            pre_var_tok_emb = pre_var_tok_embs[var_index]
            ori_var_tok_emb = ori_var_tok_embs[var_index]
            pre_ori_emb = paddle.stack(x=[pre_var_tok_emb, ori_var_tok_emb])
            var_selection = paddle.unsqueeze(x=var_selctions[var_index], axis=0)
            rename_var_tok_emb = paddle.matmul(
                x=var_selection, y=pre_ori_emb.reshape(2, -1)
            )
            rename_var_tok_emb = rename_var_tok_emb.reshape(
                tuple(pre_var_tok_emb.shape)[0], -1
            )
            rename_var_tok_embs.append(rename_var_tok_emb)
        rename_var_tok_embs = paddle.stack(x=rename_var_tok_embs)
    ...
    def get_rename_pos_and_var(self, batch):
        ...
        watermarks_class = paddle.stack(x=watermarks_class, axis=0)
        func_emb = self.node_encoder(
            batch["func_token_ids"], batch["function_token_lens"]
        )
        g_embs = self.encode_graph(
            batch["graph_batch"],
            batch["var_node_index_in_node_batch"],
            watermarks_class,
        )
        if self.config.VarDecoder["cat_func_g_emb"]:
            context = self.concat_wm_func_g_emb(
                batch=batch,
                func_embs=func_emb,
                wm_embs=None,
                g_embs=g_embs,
                only_func_g_emb=True,
            )
        else:
            context = g_embs
        var_logits = self.var_decoder(batch=batch, context=context)
        wm_embs = self.wm_encoder(batch["watermarks"])
        h = self.get_g_embs(batch["graph_batch"], batch["var_node_index_in_node_batch"])
        g_embs = paddle.mean(x=h, axis=1)
        wm_func_g_embs = self.concat_wm_func_g_emb(
            batch=batch,
            func_embs=func_emb,
            wm_embs=wm_embs,
            g_embs=g_embs,
        ...
        rename_var_tok_embs = self.get_rename_var_tok_embs(
            batch, var_selctions, var_logits
        )
    ...
    def inference(self, batch) -> Dict[str, str]:
        ...
        var_selctions = paddle.argmax(x=var_selctions, axis=-1)
        pre_var_ids = []
        ...
        sub_toks = get_var_from_var_id(
            self.config, batch["pre_var_tok_lens"], pre_var_ids
        )
